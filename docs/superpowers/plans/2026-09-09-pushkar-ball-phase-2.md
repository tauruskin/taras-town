...
| `spikes.html` | A spike patch drawn at `TOOTH_W` scale with its hit box outlined on top, at a few patch widths. This exists because of a mismatch nothing else can show: the teeth are **triangles** on a `TOOTH_W` period, and the hit box is a **flat-topped rectangle** at full height — so in the notch between two teeth the picture is at ground level while the box is still 26px tall. A ball that looks like it threaded a gap dies. Whether that reads as unfair is a thumb question, and this is where to answer it. Look at it **early** in this task, not late: if the notches need closing, it changes `SPIKE` numbers or the tooth shape, and Task 6's levels are authored against those. |
