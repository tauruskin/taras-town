...
| Page | What it is for |
|---|---|
| `panels.html` | The results panel at 844×390, 568×320 and 740×280, over grass and sky, with its tap areas ringed in magenta — **and the hidden control buttons' tap areas ringed in a second colour.** That second set is not decoration: during Task 5 the controls stayed live under the panel, and at 568×320 only 0.7px separated retry's tap area from the invisible right button's. Ringing only the panel's own buttons would hide exactly that. Open it after any change to `Panel` in `js/ui.js`. |
| `spikes.html` | A spike patch with its hit box outlined on top, **and a ball resting against the patch's left edge** — that last part is the view that answers the question. The teeth are triangles on a `TOOTH_W` period while the hit box is a flat-topped rectangle, so the picture and the thing that kills disagree. But not where you would first think: an interior notch is only `TOOTH_W` wide and the ball is `2 * BALL.R` across, so the ball can never sit in one — there is always a full-height tooth apex within half a tooth of its centre. The mismatch is confined to the patch's two **end** teeth, where the drawn face ramps up over about half a tooth, so a ball dies with its drawn edge `FORGIVE` inside the box while the nearest drawn tooth face at that height is up to half a tooth further along. Call it five to ten pixels of unfairness, at the exact place a child approaches from. Note also that `drawSpikes` lays a base strip across the whole patch, so a notch floor is a small lip rather than nothing. Whether any of this reads as unfair is a thumb question, which is why it is a page and not an assertion. Look at it **early** in this task: closing the notches would change `SPIKE` numbers or the tooth shape, and Task 6 authors its levels against those. |
...
- **The controls are dead while the results panel is up.** Input stops hit-testing the control buttons and drains held pointers, a queued jump and a queued tap on every switch into and out of the panel. Task 5 found them still live under it: at 568×320 retry's tap area sat 0.7px from the invisible right button's, and on a narrower phone a tap on retry became a held `right`. Any future overlay needs the same switch.
