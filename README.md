...
Nothing is loaded from another website, and there are no image files at all:
every tree, house, boat and character is drawn with shapes by the code. That
keeps the game tiny and means nothing can break because an external service
changed. (The home-screen icons in `icons/` are the exception, and even those
were generated by a script in `tools/`.)

Most of the sound is generated live too — the coin pings, the fanfares, the
"not yet" when something is too expensive. The exceptions are in `sounds/`:
four footsteps, a swimming stroke, a helicopter rotor and the background
music, 850KB between them. The music and the swimming were tried as generated
audio first and replaced with recordings, because short bright effects
synthesise convincingly and a melody does not. The generated versions are still in the code and still play if a
file cannot be fetched — on a first run with a bad connection, say.
...
Starts a web server and two browsers, runs **34 suites**, and shuts everything
...
| `js/audio.js` | The sound effects. Most are generated live; the footsteps and swimming play the recordings in `sounds/`, falling back to generated ones if those cannot be fetched. |
| `js/music.js` | The background music: plays `sounds/music.m4a` on a loop, with a generated tune behind it as a fallback. |
...
| `js/flight.js` | Being in the air: how high a helicopter is drawn, whether it may land where it is hovering, and the shadow underneath it. |
...
Four of them stand on open ground around the town, on painted pads, from the
very first load — like the boats moored in the river, they are there long
before anybody can afford one, because seeing one is the reason to save. A
helicopter costs **1000 coins**, the most expensive thing in the game.

Flying is **not a separate mode**: it is driving, with a vehicle that happens
to fly. Getting in takes off and getting out lands, so there is no extra
control to learn. Nothing stops a helicopter in the air except the edge of the
map — not buildings, not trees, not the river.

**Height is only ever a drawing offset.** The machine's real position stays on
the ground the whole flight, which is why its shadow needs no working out, the
map in the corner is right without any special case, and landing is a simple
question about the ground already underneath it.

**It will only land where he can also get out** — not on water, not where the
machine will not fit, and not where there is nowhere to step down onto. Where
it cannot land the button shows nothing at all rather than offering something
it would then refuse. He can always keep flying, and flying is always safe.

Fair warning: once he has one, the river, the islands and the boats all become
optional.
